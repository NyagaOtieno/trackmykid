...
// ✅ Fetch buses with expanded relations
export const getBusesWithRelations = () =>
  api
    .get("/buses?includeRelations=true")
...
// ✅ Trip start/end per session for a bus on a given day
export const getBusTrips = (
  busId: number | string,
  date?: string
) =>
  api
    .get(`/tracking/bus/${busId}/trips`, {
      params: date ? { date } : {},
    })
    .then((res) => res.data);
...
export const addBus = (busData: any) =>
  api
    .post("/buses", busData)
    .then((res) => res.data);
